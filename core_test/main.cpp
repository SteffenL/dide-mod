...
struct TestAssertError : public std::runtime_error {
    TestAssertError(const std::string& file, unsigned int line) : runtime_error(formatError(file, line)) {}
...
#define assert(expression) \
    if (!(expression)) \
    throw TestAssertError(__FILE__, __LINE__)
...
void runTests() { testSequenceSearcher(); }
