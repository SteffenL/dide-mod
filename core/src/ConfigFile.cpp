...
ConfigFile::ConfigFile(const std::string& filePath) : m_filePath(filePath) {}
...
void ConfigFile::Save(const Config& config) const { throw std::logic_error("Not implemented"); }
...
void ConfigFile::loadGeneral(Config& config) {
...

    {
        ...
        GetPrivateProfileStringW(sectionName, L"EnableMod", NULL, valueStr.data(),
                                 static_cast<DWORD>(valueStr.size() / sizeof(valueStr[0])), filePath);
    ...
    {
        std::vector<wchar_t> valueStr(1024, 0);
        GetPrivateProfileStringW(sectionName, L"EnableLogging", NULL, valueStr.data(),
                                 static_cast<DWORD>(valueStr.size() / sizeof(valueStr[0])), filePath);
        unsigned int value = 0;
        swscanf_s(valueStr.data(), L"%u", &value);
    ...
    {
        std::vector<wchar_t> valueStr(1024, 0);
        GetPrivateProfileStringW(sectionName, L"LogFile", L"dide_mod.log", valueStr.data(),
                                 static_cast<DWORD>(valueStr.size() / sizeof(valueStr[0])), filePath);
...
void ConfigFile::loadFeatures(Config& config) {
    ...
    {
        std::vector<wchar_t> valueStr(1024, 0);
        GetPrivateProfileStringW(sectionName, L"DeveloperMenu", NULL, valueStr.data(),
                                 static_cast<DWORD>(valueStr.size() / sizeof(valueStr[0])), filePath);
        unsigned int value = 0;
        swscanf_s(valueStr.data(), L"%u", &value);
    ...
    {
        std::vector<wchar_t> valueStr(1024, 0);
        GetPrivateProfileStringW(sectionName, L"CustomPak", NULL, valueStr.data(),
                                 static_cast<DWORD>(valueStr.size() / sizeof(valueStr[0])), filePath);
        unsigned int value = 0;
        swscanf_s(valueStr.data(), L"%u", &value);
...
void ConfigFile::loadCustomPak(Config& config) {
...

    {
        ...
        GetPrivateProfileStringW(sectionName, NULL, NULL, keyNamesBuffer.data(),
                                 static_cast<DWORD>(keyNamesBuffer.size() / sizeof(keyNamesBuffer[0])), filePath);
        ...
        for (wchar_t* keyName = keyNamesBuffer.data(); *keyName != L'\0'; keyName += wcslen(keyName) + 1) {
            ...
            GetPrivateProfileStringW(sectionName, keyName, NULL, pakEnabledStr.data(),
                                     static_cast<DWORD>(pakEnabledStr.size() / sizeof(pakEnabledStr[0])), filePath);
