...
DideMod::DideMod(Config config) : m_config(config) {}
...
bool DideMod::Initialize() {
    ...
    EZLOGGER << "Filesystem DLL image base: " << std::hex << std::showbase << HostAppTargetInfo::FilesystemDll
             << std::endl;

    g_mod = this;

    {
        fs_add_source_t fs_add_source = (fs_add_source_t)GetProcAddress(
            (HMODULE)HostAppTargetInfo::FilesystemDll, "?add_source@fs@@YA_NPEBDW4ENUM@FFSAddSourceFlags@@@Z");
        if (!fs_add_source) {
    ...
    {
        engine_InitializeGameScript_t engine_InitializeGameScript = (engine_InitializeGameScript_t)GetProcAddress(
            (HMODULE)HostAppTargetInfo::EngineDll, "InitializeGameScript");
        if (!engine_InitializeGameScript) {
            EZLOGGER << "Failed to get address of engine.InitializeGameScript." << std::endl;
            return false;
        }

        EZLOGGER << "engine.InitializeGameScript: " << std::hex << std::showbase << engine_InitializeGameScript
                 << std::endl;
        MH_CreateHook(engine_InitializeGameScript, engine_InitializeGameScript_detour,
                      (LPVOID*)&engine_InitializeGameScript_original);
...
void DideMod::Shutdown() { EZLOGGER << "Shutting down..." << std::endl; }
...
bool DideMod::enableDeveloperMenu() {
    ...
    EZLOGGER << "Searching for developer menu offset from " << bufferStartAddr << " to " << bufferEndAddr << "..."
             << std::endl;
...
void DideMod::logVersion() { EZLOGGER << "Core version: " << CoreGetVersion() << std::endl; }
